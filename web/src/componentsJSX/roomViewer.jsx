  ...
  return (
    ...
    >
      ...
      >
        ...
        <button onClick={handleReconnect} disabled={!ticket}>
          Reconectar
        </button>
      </div>
      <div
        ...
        {roomData ? (
          ...
          >
            ...
            >
              ...
              >
                <div>
                  <strong>ID:</strong> {roomData.id || "—"}
                </div>
                <div>
                  <strong>Nome:</strong> {roomData.name || "—"}
                </div>
                <div>
                  <strong>Turno:</strong> {roomData.turn ?? "—"}
                </div>
                <div>
                  <strong>Taxa:</strong> {roomData.tax ?? "—"}
                </div>
                <div>
                  <strong>Deck:</strong> {roomData.deck ?? "—"}
                </div>
                <div>
                ...
                <div>
                  ...
                  {roomData.gameOver ? "Sim" : "Não"}
                </div>
                <div>
                  <strong>Início:</strong> {formatDate(roomData.startTime)}
                </div>
              </div>
            </section>
            ...
            >
              ...
              {roomData.gameEvent ? (
                ...
                >
                  <div>
                    <strong>Tipo:</strong> {roomData.gameEvent.Type || "—"}
                  </div>
                  <div>
                    <strong>Jogador:</strong>{" "}
                    {roomData.gameEvent.PlayerID || "—"}
                  </div>
                  <div>
                    <strong>Expira:</strong>{" "}
                    {formatDate(roomData.gameEvent.ExpiresAt)}{" "}
                    {timeLeft && `(${timeLeft})`}
                  </div>
                  <div>
                    <strong>Payload:</strong>{" "}
                    {roomData.gameEvent.Payload
                      ? JSON.stringify(roomData.gameEvent.Payload)
                      : "—"}
                  </div>
                </div>
              ) : (
            ...
            >
              ...
              {roomData.doubledCardValues ? (
                ...
                >
                  {Object.entries(roomData.doubledCardValues).map(
                    ([cardType, data]) => (
                      <div
                        key={cardType}
                        style={{
                      ...
                      >
                        ...
                        <div style={{ marginLeft: "12px", fontSize: "13px" }}>
                          <div>
                            Rodadas até diminuir:{" "}
                            {data.roundsUntilDecrease ?? "—"}
                          </div>
                          <div>
                            Vezes dobrado: {data.timesValueDoubled ?? "—"}
                          </div>
                          <div>
                            Usado este turno:{" "}
                            {data.usedThisTurn ? "Sim" : "Não"}
                          </div>
                        </div>
                      </div>
                    ),
                  ...
                  {Object.values(roomData.players).map((player) => (
                    ...
                    >
                      ...
                      >
                        ...
                        <div>
                          <strong>Vivo:</strong> {player.alive ? "Sim" : "Não"}
                        </div>
                        <div>
                          <strong>Tempo de Movimento:</strong>{" "}
                          {player.moveTime ?? "—"}
                        </div>
                        <div>
                          <strong>Investimento Pendente:</strong>{" "}
                          {player.pendingInvestmentCoin ? "Sim" : "Não"}
                        </div>
                      </div>
                      {player.investments && (
                        ...
                        {player.cards && player.cards.length > 0 ? (
                          ...
                          >
                            {player.cards.map((card) => (
                              ...
                              >
                                <div>
                                  <strong>Índice:</strong> {card.index ?? "—"}
                                </div>
                                {card.name && (
                                  <div>
