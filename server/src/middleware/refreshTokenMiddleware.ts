    ...
    async use(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            if (decodedToken['exp'] * 1000 < Date.now()) {
                makeResponse(res, HttpStatus.UNAUTHORIZED, 'Sessão expirada. Faça login novamente.', true);
                return
            }
            ...
            if (error instanceof jwt.JsonWebTokenError) {
                makeResponse(res, HttpStatus.UNAUTHORIZED, 'Sessão inválida. Faça login novamente.', true);
                return
            }
